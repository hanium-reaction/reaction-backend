...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    mean_minutes: dict[tuple[str, str], float] = {}
    for key, scores in by_case.items():
        vals = [s["mean_minutes"] for s in scores if s["mean_minutes"] is not None]
        if vals:
            mean_minutes[key] = statistics.fmean(vals)

    deltas: dict[str, list[float]] = defaultdict(list)
    for (block, pair), value in mean_minutes.items():
        if block == CONTROL_BLOCK:
            continue
        control = mean_minutes.get((CONTROL_BLOCK, pair))
        if control is not None:
            deltas[block].append(value - control)
    ...
    scope_wins = 0
    scope_pairs = 0
    for (block, pair), _ in mean_minutes.items():
        if block != "failure_goal_scoped":
            continue
        control = mean_minutes.get((CONTROL_BLOCK, pair))
        user = mean_minutes.get(("failure_user_scoped", pair))
        goal = mean_minutes.get(("failure_goal_scoped", pair))
        if control is None or user is None or goal is None:
            continue
        scope_pairs += 1
        scope_wins += abs(goal - control) >= abs(user - control)
