...
class ExecutionEvent(Base, TimestampMixin):
    ...
    # 실행된 스케줄 블록 — DB 설계서 v0.7.1 §5.12 (plan_id 모호성 해결)
    scheduled_block_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
    ...
    # 계획 시각 vs 실제 시각 비교용 — DB 설계서 v0.7.1 §5.12
    plan_start_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    plan_end_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
