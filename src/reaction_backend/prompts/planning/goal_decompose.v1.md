...
규칙:
- 사용자 방향 최우선(가장 중요): **사용자가 밝힌 접근/자료** 가 있으면(위 '접근/자료' 가 '(없음)'
  이 아니면), 그 방향·방식·자료 구조를 **일반적 방식보다 우선**해 분해하라. 사용자가 특정 교재·
  강의·순서·방법을 언급했으면 그것을 뼈대로 삼고, 자료의 목차·주차·챕터가 있으면 그 단위로
  단계를 끊어라. '(없음)' 이면 아래 규칙대로 합리적으로 구성한다.
