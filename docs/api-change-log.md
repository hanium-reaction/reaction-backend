...
## v2.30-llm — 2026-09-18 (AI 톤 필터가 사용자 문구를 고치지 않음 · LLM 가드 정비)

요청·응답 스키마 무변경, 마이그레이션 없음.

- **사용자 문구는 금지어 치환·톤 게이트를 거치지 않는다** (api-contract §1.11, llm-1·llm-2·planB-2).
  AI 응답이 사용자 입력(인터뷰 답·목표 제목·축 제목)을 그대로 옮겨 쓴 자리는 치환하지 않고,
  톤 게이트도 그 자리의 마커로 AI 결과를 버리지 않는다. AI 가 스스로 쓴 금지어·마커는 종전대로
  치환·거절. 효과: '포기하지 않는 창업가' 답·목표 제목이 '잠깐 쉬어가는하지 않는…' 로 저장되지
  않고, 'UX 디자이너가…'·'자격증 네가지 따기'·'똑똑하게 돈 관리하기' 목표도 AI 계획
  (`aiSource="llm"`)을 받는다. 이미 깨져 저장된 값은 바뀌지 않는다(데이터 수정 없음).
  '사용자 문구'로 보는 프롬프트 변수는 **이름을 적어둔 목록**뿐이다 — 목표 슬롯·인터뷰 답·
  인박스 원문·검색어·궁극목표 슬롯·사용자가 편집한 만다라 제목. 참고 자료 본문(`materials`)은
  사용자가 붙여넣은 링크를 서버가 열어 가져온 **제3자 웹페이지 본문**을 담을 수 있어 목록에서
  제외했다. 목록에 없는 변수는 보호하지 않으므로 필터가 종전대로 걸린다.
- **계획·만다라트 일일 요청 상한이 상한 대상 엔드포인트만 센다** (api-contract §1.10, llm-4).
  자료 검색(그라운딩 예산에 걸려 거절된 것 포함)·마일스톤 초안·학습 방식 추천·만다라 링
  재생성은 더 이상 `/plans/generate` 등의 상한을 깎지 않는다. 429
  `RATE_LIMIT_DAILY_CALLS_EXCEEDED` 의 `message` 가 탓하지 않는 문구로 바뀐다 —
  "오늘 준비된 {기능} 횟수를 다 썼어요. 내일 다시 열려요." (코드·`Retry-After` 무변경).
- **LLM 예산 검사가 이번 프롬프트 크기를 미리 더한다 + 호출 1회 프롬프트 상한**
  (api-contract §1.10, llm-5). 사용자별·전역 토큰 예산 검사에 추정 입력 토큰(글자 수의 2/3)을
  더하고, `LLM_MAX_PROMPT_CHARS`(신규 설정, 기본 60,000자, 0=무제한)를 넘는 프롬프트는
...
## v2.30-calendar — 2026-09-18 (캘린더 연결이 조용히 끊기지 않게 · 고정 일정 겹침 409·24:00)

**additive + 동작 변경 — `GET`/`POST`/`DELETE /calendar/connect` · 계획 생성/재계획 `warnings` ·
`POST`/`PATCH /fixed-schedules`.** 마이그레이션 없음. 기존 필드·에러 코드 무변경.
...
### 고정 일정 (`/fixed-schedules`, §19)

- **겹침 409 `FIXED_SCHEDULE_OVERLAP` 을 이제 실제로 낸다** — 계약에는 있었는데 서버가 검사하지 않아,
  느린 연결에서 [추가] 를 두 번 누르면 같은 수업이 두 줄 생겼다(오늘 화면에도 두 번). 같은 요일에
  시간이 걸치면 409, 맞닿기만 하면 통과. 검사·저장은 사용자별 lock 안이라 연타도 한 줄만 생긴다.
  PATCH 는 요일·시각을 바꿀 때만 검사한다(예전에 겹쳐 저장된 일정도 제목은 고칠 수 있게).
- `endTime: "24:00"` 을 받는다(그날 끝까지 막는다 — 시간 정책·스케줄러와 같은 규칙). 응답에는 `"23:59"`.
- 자정을 넘는 일정(`22:00–02:00`)은 여전히 422 지만, 이유와 입력 방법을 말한다: "자정을 넘기는 일정은
  둘로 나눠 넣어 주세요. 예: 금 22:00–24:00, 토 00:00–02:00". 같은 요일로 접으면 금요일 새벽을 막고
  토요일 새벽을 비워 두기 때문에 그대로 받지 않는다 — 다음 요일로 넘기는 전개는 별도 결정이다.
...
## v2.30-today — 2026-09-18 (오늘 실행 루프 — 이어서 하기·자정 넘긴 카드·취소 블록 정리·쪼갠 카드·정지 멱등·실제 소요 시간)

마이그레이션 없음. 에러 envelope 무변경. 에러 코드 추가·삭제 없음.

### `POST /today/actions/{id}/start` — 같은 카드면 진행 중인 실행을 돌려준다 (409 → 200)

**무엇이 막혀 있었나.** FE 는 실행 id 를 sessionStorage 에만 들고 있다. 앱이 백그라운드에서
죽거나 탭을 닫으면 그게 비고, 오늘 화면의 [이어서 하기] 는 start 를 다시 부른다. 서버는 같은
카드의 실행이 이미 있으니 409 `TODAY_EXECUTION_ALREADY_ACTIVE` 를 돌려줬고, '다시 시도' 도 매번
409 — [완료] 는 "시작 기록이 저장되지 않아 완료할 수 없어요" 로 영영 막혔다. 끝낸 일을
'일부만/잘 안됐어요' 로만 남길 수 있어 회복 제안·주간 준수율까지 왜곡됐다.

**이제**
- 같은 카드에 진행 중(in_progress) 실행이 있으면 **그 실행을 200** 으로 돌려준다. 새 실행·블록을
  만들지 않고 카드 상태도 건드리지 않는다. 응답 모양은 201 과 같다.
- `actualStartAt` 은 **처음 시작한 시각** — FE 는 이걸로 타이머를 00:00 이 아니라 실제 경과부터
  이어 붙일 수 있다.
- 체크인으로 끝난 실행은 되살리지 않는다 — 그 뒤 start 는 종전대로 새 실행(201).
- 다른 카드가 진행 중이어도 시작은 막지 않는다(종전과 같음). `TODAY_EXECUTION_ALREADY_ACTIVE`
  코드 정의는 남아 있지만 이 경로는 더 이상 내보내지 않는다.

**FE 가 할 일.** 수정 없이도 [이어서 하기] 가 풀린다(200 도 성공으로 읽으므로 돌려받은
`executionId` 를 그대로 쓴다). 권장: 카드가 `status='in_progress'` 이고 `executionId` 가 있으면
start 를 부르지 않고 그 id 를 바로 쓰고, 타이머는 start 응답의 `actualStartAt` 부터 잰다.

### `GET /today/agenda` — 자정을 넘겨도 하던 카드가 남는다 (additive: `AgendaCard.carriedOver`)

**무엇이 비어 있었나.** 어젠다는 `target_date == 오늘` 만 모으는데 `target_date` 는 블록의 KST
시작일이고, 계획은 블록을 자정 너머로도 놓는다. 23:40 에 시작한 카드는 00:00 이 되는 순간 오늘
화면에서 사라졌고(열어 둔 화면도 60초 폴링이 지웠다), 23:30~00:30 블록을 00:05 에 늦게 시작하려
해도 카드가 없었다. 밤에 공부하는 대학생에게 특히 잦다.

**이제**
- `AgendaCard.carriedOver` (bool, 기본 `false`) 추가. 오늘 카드 **뒤에** 다음 카드를 이어 붙이고
  `true` 로 표시한다(보관 카드 제외, 오늘 카드와 중복 없음):
  1. 진행 중 실행이 회고 창 안에 있는 지난 날짜 카드 — 창 기준은 `/reflection/pending` 과 같아서
     두 화면이 같은 카드를 보고, 창을 벗어나면 종전대로 만료 cron 이 정리한다. `executionId` 로
     바로 체크인할 수 있다.
  2. 어제 시작해 아직 안 끝난(미종결) 블록이 있는 카드 — 늦게라도 [▶ 시작] 할 수 있게.
- 체크인으로 끝나면 다음 조회부터 빠진다. 기존 필드·정렬(오늘 카드끼리의 순서) 무변경.

**FE 가 할 일.** 수정 없이도 카드가 계속 보인다. 권장: `carriedOver=true` 카드에 '어제 이어서'
같은 작은 표시를 얹는다.

### `POST /today/actions/{id}/cancel` — 취소한 카드의 블록도 함께 정리된다

**무엇이 남아 있었나.** 취소는 카드의 `archived_at` 만 세팅했다. 주간 재계획으로 블록이 잡힌
카드를 취소하면 블록이 `scheduled` 로 영원히 남아 — 주간 그리드에 취소한 카드 제목으로 뜨고(눌러
보면 404), 그 시간대는 계속 '바쁨' 이라 다른 블록을 옮겨 오면 422 `PLAN_BLOCK_CONFLICT` 로
막혔다. 정리해 주는 cron 도 없었다.

**이제** 같은 트랜잭션에서 그 카드의 미종결 블록(`scheduled`/`started`)을 `cancelled` 로 바꾼다.
응답(204)·멱등(재호출 204)·`status` 불변은 그대로. `finished` 블록은 건드리지 않는다.

⚠️ **이미 남아 있는 유령 블록은 이 변경으로 사라지지 않는다** — 소급 정리(보관된 카드의
`scheduled` 블록 cancel, dry-run 먼저)는 운영 스크립트로 따로 한다.

### `GET /today/agenda` · `POST /today/check-ins` — 여러 날로 쪼갠 카드

**무엇이 어긋나 있었나.** 주간 '남은 일 다시 배치' 는 긴 카드(예: 90분)를 월·수 45분씩처럼 여러
날의 세션 블록으로 쪼갠다. 카드는 1장이고 `target_date` 는 가장 이른 블록의 날짜 하나라 —
둘째 날부터는 카드가 오늘 화면에서 사라져 시작할 방법이 없었다. 첫 회차에서 '완료' 하면 카드는
done 인데 남은 회차 블록은 `scheduled` 로 남아 주간표에 할 일처럼 뜨고, 5분 전 '곧 시작' 알림까지 왔다.

**이제**
- 어젠다 `cards` 는 target_date 가 오늘인 카드에 더해 **오늘(KST) 시작하는 취소 안 된 세션
  블록**이 있는 카드를 오늘 카드로 싣는다(`carriedOver=false`, 중복 없음). 오늘 회차를 체크인해도
  (`finished`) 오늘 화면에 남는다. 응답 모양 무변경.
- 체크인·저녁 회고가 `done`/`over_done` 이면 그 카드의 남은 `scheduled` 회차 블록을 `cancelled`
  로 바꾼다. 수행 이력(`finished`)·진행 중(`started`)·사용자가 옮긴(`user_edit`) 블록은 그대로.
  `partial_done`/`failed` 는 남은 회차를 그대로 둔다(다음 회차에 이어서 한다).
- pre_card 알림은 끝낸(`done`/`over_done`) 카드의 블록엔 보내지 않는다.

**FE 가 할 일.** 수정 없이 둘째 날에도 카드가 보인다. ⚠️ 지금 FE 는 `partial_done` 카드를 '회복
대기' 로 그려 누르면 회복 제안으로 보낸다 — 쪼갠 카드의 둘째 날엔 그 카드가 `partial_done`(첫 회차를
'조금 함' 으로 남긴 경우)일 수 있으니, 오늘 블록이 남아 있는 카드는 [▶ 시작] 을 먼저 보여 주는 편이
맞다. 회차 표시('(2/2)')가 필요하면 별도 이슈로 필드를 추가한다.

### `POST /today/focus/{id}/pause`·`/resume` — 멱등 (409 → 200)

**무엇이 막혀 있었나.** 집중 화면에서 '잠시 멈추고 나갈게요' 로 정지한 뒤 6시간이 지나면 cron 이
그 정지를 '안 돌아옴' 으로 표시했고, 서버는 그걸 닫힌 정지로 봤다. 저녁에 돌아와 [계속] 을 누르면
409 `TODAY_NOT_PAUSED` — FE 는 '서버에 저장되지 않았어요. 다시 시도해 주세요' 를 띄우고, 다시
시도해도 매번 409 였다. 멈춘 몇 시간은 정지 시간에 한 번도 안 들어갔다. 정지 응답만 잃은 경우의
재전송도 409 `TODAY_ALREADY_PAUSED` 로 같은 고리에 빠졌다.

**이제**
- pause: 이미 정지 중이면 새 구간 없이 **200 `status=paused`**.
- resume: 정지 중이 아니면 아무것도 안 바꾸고 **200 `status=in_progress`**.
- 6h cron 표시(`resumed_after_interrupt=false`)는 사실 기록으로 남기고 정지는 열린 채로 둔다 —
  저녁의 [계속] 이 재개되고, 멈춘 시간이 `pauseTotalMinutes` 에 들어간다.
- 에러 코드 정의는 그대로(삭제 없음). 체크인이 끝난 실행 409·없는 실행 404 는 종전과 같다.

**FE 가 할 일.** 수정 없이 '다시 시도' 배너가 풀린다. 권장: 응답의 `status` 로 화면 상태를 맞춘다.

### 실제 소요 시간(`actualDurationMinutes`)이 일한 시간만 센다

**무엇이 부풀어 있었나.** 체크인은 소요 시간을 '시작 ~ 체크인' 전체로 셌다 — 09:00 에 시작해
09:20~18:00 멈췄다가 18:10 에 완료하면 550분. 정지 중에 바로 체크인하면 그 정지는 영영 열린 채로
남았다. 저녁 회고는 회고한 시각을 끝낸 시각으로 써서, 13:00 에 시작한 30분 카드를 21:30 에 '완료'
하면 510분이었다. 이 값이 주간 리뷰 `effort.actualMinutes`('실제로 쓴 시간')로 간다.

**이제**
- `POST /today/check-ins`: `actualDurationMinutes` = (체크인 − 착수) − `pauseTotalMinutes`. 정지 중
  체크인은 그 정지를 체크인 시각에 닫고(재개 없이 끝냄) 정지 시간에 더한 뒤 뺀다.
- `POST /reflection/batch`: 소급 종결이라 소요 시간을 **비워 둔다**(null) — 지어내지 않는다.
...
## v2.30-planA — 2026-09-18 (계획 초안 편집이 승인에 반영된다 외)

마이그레이션 없음. 응답 envelope·기존 필드·에러 코드 무변경 — 모두 additive.

### `POST /plans/{planId}/approve` — 선택 본문 `blocks` (HITL '수정')

**무엇이 잘못됐었나.** 초안 화면(온보딩 4/4)이 "탭하면 수정 · 끌면 이동" 을 안내하는데, 승인
요청은 `planId` 만 보냈고 서버도 본문을 안 받았다. 옮기고·지우고·이름 바꾼 것이 전부 버려지고
AI 원안이 그대로 저장됐다 — 사용자는 캘린더에서야 알게 된다.

**이제.** `{ blocks?: [{ originId, start, end, title? }] }` 를 보내면 그것이 최종 블록 목록이다.
옮긴 시각으로 저장하고, 블록이 모두 빠진 카드는 만들지 않고, 바꾼 제목은 카드 이름으로 저장한다.
옮긴 블록만 기존 일정·고정 일정·시간 정책·구글 캘린더 약속과 다시 대조한다
(422 `PLAN_BLOCK_CONFLICT` / `PLAN_POLICY_VIOLATION`). 초안에 없던 `originId` 는 422 `COMMON_VALIDATION_ERROR`. 본문이
없거나 `{}` 면 종전과 같다.

**시각은 15분 snap 하지 않는다(리뷰 반영).** 스케줄러가 놓는 초안 시각은 15분 격자가 아니다 —
쉬는 시간 10분 뒤 2회차는 :10 에 시작하고, 수업이 10:50 에 끝나면 블록도 10:50 에 시작하고,
10분짜리 회차도 있다. 처음 구현은 받은 시각을 전부 snap 한 뒤 초안과 비교해서, 손대지 않은
초안을 그대로 보내도 10:50 블록이 10:45 로 당겨져 수업과 겹친다며 422, 13:10~14:10 이
13:15~14:15 로 저장(쉬는 시간 소실), 12:40~12:50 이 12:45~12:45 가 돼 422 였다. 이제:
- 그 카드의 초안 칸과 **분 단위까지 같은** 블록은 손대지 않은 것 — 초안 시각 그대로 저장하고
  다시 검사하지 않는다. 같은 초안 칸을 두 번 보내면 422 `COMMON_VALIDATION_ERROR`.
- 다르면 옮긴 블록 — **보낸 시각 그대로**(분 단위, 초·밀리초는 뗀다) 저장한다. 초안 화면의
  끌기는 "원래 시각 + 15분 단위" 라 10:50 블록을 한 시간 끌면 11:50 이 보인다. snap 하면
  사용자가 승인한 화면과 다른 11:45 가 승인 뒤에야 저장된다(PATCH 편집은 snap 결과를 바로
  응답해 화면이 따라가지만 승인에는 그런 확인 단계가 없다).

**옮긴 블록은 캘린더 약속도 피한다(리뷰 반영).** 계획 생성은 연결된 구글 캘린더의 약속을
피해 블록을 놓는데(ADR-0009 D4) 승인의 재대조에는 그 소스가 빠져 있었다 — 사용자가 끌어 옮긴
블록만 약속 위에 얹혀 저장됐다. 이제 옮긴 블록의 날짜 범위를 한 번 조회해 겹치면 422
`PLAN_BLOCK_CONFLICT` 다(약속 제목은 싣지 않는다 — busy 조회라 서버도 모른다). 미연결이거나
조회가 실패하면 그 대조만 건너뛰고 승인은 그대로 진행한다(생성과 같은 규칙 — 캘린더 장애가
승인을 막지 않는다).

**FE 가 할 일(reaction-frontend 이슈).** `WeeklyPlanGenerationScreen` 의 승인 호출에 편집된
블록 목록을 싣는다 — `originId` 가 있는 블록만(초안에서 `originId` 가 없는 블록은 원래 저장되지
않는다). 나뉜 회차가 같은 `originId` 를 공유하므로 화면 상태의 키는 `originId + 회차 index`
처럼 **블록마다 달라야** 한다(지금은 한 회차를 옮기면 같은 카드의 모든 회차가 같이 움직인다).
'블록 추가'로 만든 새 블록은 이 본문으로 저장할 수 없다 — 추가 버튼을 숨기거나 "시작 후
캘린더에서 추가" 로 안내한다.
손대지 않은 블록은 초안 응답의 `start`/`end` 를 **그대로** 돌려보낸다 — 화면 모델의 `dur` 은
최소 15분으로 올려 잡으므로(`previewToBlock`) 끝 시각을 `time + dur` 로 다시 계산하면 10분
회차가 15분짜리 "옮긴 블록"으로 저장된다.

### 재시도 루프 (동작 수정, 계약 무변경)

같은 endpoint 의 3회 재시도가 실 DB 에서 한 번도 돌지 못했다 — 첫 실패의 rollback 이 `user`
인스턴스를 만료시켜 다음 시도가 일반 500(`COMMON_INTERNAL_ERROR`)으로 터졌다. 이제 문서대로
재시도하고, 다 실패하면 500 `PLAN_SAVE_FAILED` 다.

### `POST /plans/replan` — 사용자의 활동 시간대를 지킨다 (동작 수정, 계약 무변경)

**무엇이 잘못됐었나.** 첫 계획은 활동 시간대(인터뷰 답 + 설정에서 고친 값)의 여집합을 수면으로
보고 피하는데, 재계획은 DB `time_policies` 만 봤고 비어 있으면 23:00~08:00 기본값을 썼다. FE 에는
`time_policies` 를 만드는 경로가 없어 실사용자는 늘 기본값이었다 — 저녁에만 된다고 답한 학생도,
새벽형도 "남은 일 다시 배치" 한 번에 아침 7~8시로 블록이 몰렸다.

**이제.** 재계획도 첫 계획과 같은 조립이다: 최근 완료 인터뷰 outcome(설정에서 고친 활동 시간대
반영)의 활동창 밖 + no_touch + DB 시간 정책. 인터뷰가 없으면 설정의 활동 시간대
(`focus_mode_preferences.activity_start/end`) 밖을 피하고, 그마저 없을 때만 23:00~08:00 기본값이다.

### `POST /plans/replan` — 남겨 두는 예정 블록 위에 겹쳐 잡지 않는다 (동작 수정, 계약 무변경)

형제 세션을 착수했거나 사용자가 옮긴 카드는 통째 보존되는데, 그 카드의 나머지 **예정** 회차는
회피 대상에 없었다(확정 = 시작/완료·`user_edit` 만). 그래서 다른 카드가 그 위에 잡혀 승인 후
같은 시간 블록 두 개가 생겼다. 이제 이번 재계획이 교체하지 않는 블록은 모두 피한다.

### `POST /plans/replan` — 지운·완료한 목표의 카드는 다시 배치하지 않는다 (동작 수정, 계약 무변경)

후보 조회가 카드의 보관 여부만 보고 목표 상태는 안 봐서, 지운 목표에 남아 있던 카드를 '남은
일' 로 다음 주부터 새로 깔았다. 이제 보관(삭제)·완료된 목표의 카드는 후보에서 뺀다(지울 때
카드·블록을 정리하는 것은 `DELETE /goals/{id}` 쪽 수정). 목표 없는 인박스 카드는 그대로 대상이고,
보류(parked) 목표는 한도 초과 자동 보류도 카드가 살아 있는 계약이라 빼지 않는다.

### `POST /plans/replan` — 목표마다 자기 마감 안에 배치한다 (동작 수정, 계약 무변경)

모든 후보를 지평 하나에 균등 분산해서, 금요일 시험 목표의 남은 세션이 한 달짜리 목표의 지평에
섞여 절반이 시험 뒤로 밀렸고 아무 말도 없었다. 이제 목표 마감(`goals.deadline`)이 지평보다 이른
카드는 마감이 이른 목표부터 자기 마감(포함)까지 배치한다. 마감 전에 다 못 넣은 세션은 마감 뒤로
밀지 않고 `warnings` 에 목표 단위 한 줄로 알린다 — "'토익' 마감(9월 25일) 전에 빈 시간이 모자라
2개 일정은 넣지 못했어요. …". 응답 형태는 그대로다.
마감이 재배치 시작일(`windowStart`) 전에 **이미 지난** 목표의 카드는 넣을 마감 안 자리가 없다 —
버리지 않고 전체 지평에 배치하되 "'토익' 마감(9월 12일)이 이미 지나서, 남은 일정은 그 뒤로 잡아
뒀어요. 계속할 목표라면 마감을 새로 정해 주세요." 를 목표 단위 한 줄로 싣는다(리뷰 반영 — 예전엔
아무 말 없이 마감 뒤로 흩어졌다).

### `PATCH /plans/{planId}/blocks/{blockId}` — 시작/끝낸 블록은 시간을 못 옮긴다 (동작 수정)

끝낸 블록을 다음 주로 끌면 수행 기록이 미래로 가고, 카드 날짜가 따라 움직여 오늘 끝낸 카드가
오늘 화면에서 사라졌다. 이제 `blockStatus` 가 started/finished 인 블록을 다른 시각으로 보내면
422 `PLAN_INVALID_TIME`("이미 시작했거나 끝낸 일정은 옮길 수 없어요. 제목이나 목표만 바꿀 수
...
### 계획 생성·재계획 — '내 정보'에서 고친 집중 길이·집중 시간대를 쓴다 (동작 수정, 계약 무변경)

설정 화면은 "여기서 바꾸면 인터뷰를 다시 하지 않아도 반영돼요" 라고 하는데, 계획은 인터뷰 답만
읽어 `PATCH /settings/profile` 의 `attentionSpan`·`energyCycle` 수정이 어디에도 안 닿았다(활동
시간대만 반영됐다). 이제 `POST /plans/generate`·`/plans/milestones`·`/plans/replan`·
`/plans/mandala/next-cycle` 이 인터뷰가 **끝난 뒤에** 바뀐 프로필 값을 얹는다 — 인터뷰 답에서 나올
값과 다를 때만. 집중 길이를 고치면 목표별 세션 길이 대신 그 값으로 세션을 나누고, 집중 시간대는
전역 피크만 바꾼다(목표별 선호 시간은 더 구체적인 답이라 유지).

**FE 가 할 일(reaction-frontend 이슈).** `MyInfoScreen` 의 '알림 빈도'는 아직 어느 알림에도 쓰이지
않는다(잠금된 3종 알림 정책 안에 자리가 없다) — 컨트롤을 숨기거나 비활성화하고, "인터뷰를 다시 하지
...
### `POST /plans/replan` — 이번 주로 잡아 둔 블록 없는 카드는 그대로 둔다 (동작 수정, 계약 무변경)

백로그(활성 블록 없는 planned 카드)를 날짜와 무관하게 전부 후보로 집어서, 인박스에서 '할 일로'
...
⚠️ **1:1 매핑이 아니다(리뷰 반영).** 한 카드가 여러 회차로 나뉘면 새 회차 **전부**가 같은 값,
즉 그 카드의 **대표(첫) 옛 블록** 시각을 싣는다(밀린 카드면 그 첫 블록이 과거의 밀린 블록이다).
"이 회차의 원래 시각" 이 아니라 "이 카드가 원래 있던 자리" 로 읽어야 한다 — 회차가 여럿이면
"기존 M월 D일 HH:MM 일정을 N개 회차로 나눠 옮겼어요" 처럼 카드 단위로 보여 주는 게 맞다.
...
- 같은 PATCH 에서 **시각을 그대로 보낸 제목·목표 편집**(`startAt`·`endAt` 이 지금 블록 시각과 같음)은
  15분 snap 도, 겹침·정책·고정 일정 검사도 하지 않는다(리뷰 반영). 예전엔 스케줄러가 격자 밖에 놓은
  10:50 블록의 제목만 바꿔도 10:45 로 옮겨졌고, 나중에 추가한 수업 위에 이미 있던 블록은 이름조차 못
  바꿨다(422). 시간을 옮기는 편집은 종전 그대로 snap·검사한다.
...
로그인은 원래 게이트를 안 거쳐 영향 없음.

### 무엇이 막혀 있었나

실서버 확인(`toggle-signups.yml` show) 결과 가입 인원 **76 / 30**, 초대코드 **0개 발급** —
신규 가입이 두 겹으로 막혀 있었다. 76명 중 67명은 예전 스테이징 stub 로그인이 만든
`@reaction.local` 데모 계정이고 실제 Google 계정은 9명뿐이다.

### 이제

- `SIGNUP_CAPACITY` 미설정 = **상한 없음**(기본값). 숫자를 넣으면 종전대로 403
  `AUTH_SIGNUP_CAPACITY_REACHED`.
- `SIGNUP_INVITE_REQUIRED`(신규, 기본 `false`) — 꺼져 있으면 `inviteCode` 를 **무시**한다
  (검증·소비 안 함, 보내도 가입 성공). `true` 면 종전 동작(422/409).
- `SIGNUPS_ENABLED`(긴급 차단)는 그대로.
...
- **재계획만 캘린더를 안 읽었다.** v2.15 에서 첫 계획(`POST /plans/generate`)에 캘린더를 busy
  소스로 배선했지만 재계획의 busy 는 확정 블록·시간정책·고정일정 셋뿐이었다. 캘린더를 연결한
  사용자가 재계획을 누르면 카드가 수업·약속 위에 다시 잡혔다.
- **토큰 갱신이 계획 생성의 lock 을 풀었다.** `freebusy` 가 토큰을 갱신하면 곧바로 commit 했는데,
  계획 생성은 트랜잭션 단위 advisory lock(`pg_advisory_xact_lock`) 안에서 이걸 부른다. commit 은
  그 lock 을 해제한다 — access token 이 만료되는 한 시간마다 한 번, 계획 생성이 **도중에 lock 을
  잃고** 나머지를 보호 없이 진행했다.

### 이제

- 재계획이 재배치 창 전체의 캘린더 일정을 **한 번** 조회해 회피 대상에 넣는다(출처 `calendar` —
  하루 상한·여백 계산 대상인 확정 블록과는 섞이지 않는다, 첫 계획과 같은 규칙).
- `warnings`(연결한 사용자에게만):
  - 못 읽음 → 첫 계획과 같은 "캘린더 일정을 불러오지 못해서 이번 계획에는 반영하지 못했어요…"
  - 지평이 조회 상한(60일)보다 김 → "캘린더 일정은 M월 D일까지만 반영했어요. 그 뒤에 잡힌 카드는
    캘린더와 겹치는지 확인해 주세요." (재계획 지평은 1년까지 벌어진다)
- `freebusy` 는 **flush 까지만** 한다. 갱신 토큰·회수 표시는 호출자의 commit 에 실린다.
  `GET /calendar/freebusy` 는 lock 이 없어 조회 후 스스로 commit 한다.

---

## v2.25 — 2026-09-16 (「나중에」 한 번으로 재협상 모드가 뜨지 않는다, #479)

**동작 보정(스키마·응답 형태는 그대로) — `POST /recovery/proposals/generate` 의
`recoveryMode="goal_renegotiation"` 판정.** 마이그레이션 없음.

- L3 조건 "회복 2회 연속 rejected/skipped"(v1.87)가 **회복 결정 횟수가 아니라 카드 행 수**를
  셌다. 한 번의 결정은 그 세트의 카드 2~4장을 함께 닫으므로, 신규 사용자가 2장 세트에
  「나중에」를 **한 번** 누르면 다음 생성부터 `goal_renegotiation`(+ `aiSource: "rule"`)이
  나왔다. 사용자 단위 이력이라 **다른 카드의 회복도** 수락 전까지 전부 그랬다.
- 이제 결정 1회(같은 실행에서 같은 시각에 닫힌 카드 묶음)를 1회로 센다. 수락·편집 수락은
  형제 카드의 자동 `rejected` 와 섞여 거절로 읽히지 않는다. 문턱(2)·사용자 전체 스코프·다른
  L1/L2/L3 조건은 그대로다.
- FE 영향: 실제로 두 번 미루기 전까지 재협상 헤더·카피·"오프라인 모드" 안내가 뜨지 않는다.
  필드·값 집합 변경은 없다.
...
`GET /reviews/weekly` 의 `nextCycleProposals` 는 `goalId` 를 주는데, 그 카드가 안내하는 승인
경로(`POST /plans/generate`)는 **목표를 받지 못하고** 최근 완료 인터뷰를 자동 재투영했다.

목표를 여러 개 굴리는 사용자에게는 그게 다른 목표일 수 있다 — 제안 카드에서 **목표 A** 의
다음 주기를 열었는데 **최근 인터뷰 목표 B** 의 계획이 생성되고, 그대로 승인까지 갈 수 있었다.
`docs/api-contract.md` 에 "미해결" 로 적혀 있던 위험이다.

### 이제

`goalId` 를 주면 그 목표로 계획이 선다. `nextCycleProposals[].goalId` 를 그대로 넘기면 된다.

- 가용 시간·선호·정체성은 **인터뷰 답 그대로** 두고 `core_goals` 만 갈아끼운다. 그 목표에
  대해 이미 답해 둔 슬롯(주당 시간·세션 길이·선호 시간대)이 있으면 **보존한다** — 목표를
  지정했다는 이유로 사용자가 답한 값이 사라지면 안 된다.
- 만다라 승격 목표의 **2주 지평도 그대로** 걸린다. 새 규칙을 넣어서가 아니라
  `_max_plan_weeks` 가 heaviest **제목**으로 판정하기 때문이다 — '2주'가 두 곳에서 따로
  정의되지 않는다.

### 오류

- 없는 목표 · **남의 목표** · 보관된 목표 → 404 `GOAL_NOT_FOUND`. 세 경우를 한 갈래로 묶은
  이유는 "있지만 권한 없음" 을 돌려주면 존재 여부가 새어 나가서다.
- **완료한 목표 → 422** `COMMON_VALIDATION_ERROR`(`field="goalId"`). 404 로 묶지 않았다 —
  목표는 실제로 있고 목록에도 보이므로 "없다"는 거짓말이 된다. 되돌린 뒤 다시 세우라고 안내.
- `proposed`(계획 미승인 잠정 목표)는 **막지 않는다** — 인터뷰가 목표를 proposed 로 먼저
  저장하므로(#96) 첫 계획을 세우는 정상 경로가 그 상태다.

### 곁들여 정리한 것

`core_goals` 를 갈아끼우는 규칙이 `mandala_cycle`(만다라 축 전용)에 있었는데, 축
(`GoalNode`)은 #373 에서 `why_text` 읽기가 빠진 뒤로 그 규칙에 **아무 값도 기여하지 않고**
승격된 `Goal` 만 쓰이고 있었다 — 파라미터가 죽어 있었다. 처음부터 "축"이 아니라 "목표" 단위
규칙이라 `orchestrator/goal_cycle.py` 로 옮겨 두 입구(만다라 축 · 다음 주기 제안)가 같은 한
벌을 쓰게 했다. 만다라 경로는 동작 무변경.

---

## v2.22 — 2026-09-04 (목표를 완료하면 만다라 유래 카드와 회복 카드도 멈춘다, #367)

**동작 변경 — `POST /goals/{goalId}/complete`.** 응답 스키마 무변경. 마이그레이션 없음.

### 무엇이 잘못됐었나

v1.92(목표 완료 시 남은 카드 정리)가 재사용한 `first_plan_adapter.supersede_previous_plan`
에는 **승인 경로의 사정**이 함께 들어 있었다 — 계획 재생성이 만다라 73칸(W2,
`1ee508b967ba`)과 회복 카드까지 쓸어가면 안 된다는 보호 규칙이다.

완료 경로에는 그 사정이 없다. 사용자가 "이 목표 끝났어요" 를 눌렀는데 만다라에서 온 카드만
계속 뜨는 건 의도가 아니다. 그런데 **함수를 재사용하면서 규칙도 같이 딸려와** 새 호출부에서
틀린 답을 냈다:

- **궁극목표는 카드가 전부 만다라 유래라 한 장도 정리되지 않았다** — 완료를 눌러도 증상이
  100% 그대로 남는다.
- 만다라 축에서 승격된 목표도 그 축 유래 카드는 안 멈췄다.
- 회복 카드는 `create_from_recovery` 가 `goal_id` 를 아예 안 채워서 **어느 목표를 완료해도**
  안 멈췄다.

### 이제

정리 규칙을 함수에 못 박지 않고 **호출부가 고른다** —
`supersede_previous_plan(include_mandala=..., include_recovery=...)`. 둘 다 기본값 `False` 라
**승인 경로는 동작이 완전히 같다**(회귀 가드 테스트 2건 추가). 완료 라우트만 둘을 켠다.

완료 전용 함수를 따로 두지 않은 이유는 `protected_card_ids`(사용자가 옮긴 블록 보존) 같은
**공유 보호 규칙이 두 벌로 갈리는 것**이 더 위험해서다.

`ActionItemRepo.create_from_recovery` 는 이제 **부모 카드의 `goal_id` 를 물려받는다.** 회복은
'그 목표를 계속하는 다른 방법'이지 목표 밖의 일이 아니다. 부모에 `goal_id` 가 없으면
(inbox/manual 유래 실패 카드의 회복) 그대로 `None` — 지어내지 않는다.

### 보존되는 것은 그대로

시작·완료·실패한 카드, 사용자가 시간을 옮긴(`user_edit`) 블록을 가진 카드, 직접 만든/인박스
카드는 완료 경로에서도 여전히 남는다.

### ⚠️ 한계 — 옛 회복 카드

**v2.22 이전에 만들어진 회복 카드는 `goal_id` 가 비어 있어 계속 안 걸린다.** 백필은 데이터
마이그레이션이라 AGENTS §8("사람에게 먼저 물어볼 것")에 걸려 이 PR 에 넣지 않았다 — 필요
여부는 #367 에서 논의한다. 참고로 `recovery_attempts` 는 2026-08-17 실측에서 0건이었다(#258).
...
직접 생성 경로(`POST /goals`)는 `goals.py::_enforce_tier_limit` 로 Focus≤3/Maintain≤5 를
지키지만, 목표가 실제로 만들어지는 주 경로인 **계획 승인**에는 한도 검사가 없었다.
`first_plan_adapter.materialize_goals` 가 인터뷰의 `tentative_tier` 를 그대로 active 로
승격시켜, 승인을 반복하면 한도를 얼마든지 넘길 수 있었다(실측: 배포 계정에서 "집중 4/3").
화면에는 "가득참" 배지만 뜨고 왜 넘었는지는 안 보였다.

### 이제

`first_plan_adapter._park_tier_overflow_on_approval` 가 승인 트랜잭션 안에서 한도를
재검사한다.

- **승인 자체는 막지 않는다** — 422 로 거절하면 사용자가 이미 끝낸 인터뷰가 통째로
  버려진다. 대신 한도를 넘긴 만큼만 `parked` 로 조용히 돌린다.
- **이번 승인이 새로 active 로 올린 목표만** 대상이다 — 이미 active 였던 다른 목표는
  이번 승인과 무관하므로 손대지 않는다.
- **heaviest(이번에 실제로 분해·배치된 목표)는 최후순위로 보호된다** — 다른 후보가 있으면
  그쪽부터 내린다. `goal_tier` 는 목표 화면 분류일 뿐 오늘 카드·실행을 가리지 않으므로
  parked 로 돌려도 안전하지만, 방금 카드까지 받은 목표부터 내리면 당장 혼란스럽다. 다만
  기존 데이터가 이미 한도를 넘겨 있어 heaviest 하나뿐이면 그것도 내린다 — 한도를 못
  지키는 예외를 두지 않는다.
- 내린 목표 제목이 응답의 `warnings`(`list[str]`, 대개 빈 배열)에 실린다.

### 호환

- `warnings` 는 새 필드다. 기본값 `[]` — 안 읽어도 종전과 같다.
