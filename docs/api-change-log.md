...
`POST /replan/{executionId}/approve`.** 마이그레이션 없음. 새 에러 코드 없음.

v2.31 이 **생성**의 동시성을 닫은 뒤, 그 다음 두 쓰기 경로에도 같은 교차가 남아 있었다.
Idempotency-Key 미들웨어는 끝난 응답만 재생하므로 더블탭·재시도로 겹친 요청은 둘 다 실행됐다.

- **결정**: 수락 두 번이 겹치면 회복 ActionItem 이 두 개 생겼다(하나는 고아 카드로 오늘
  화면에 남는다). 수락과 「나중에」가 겹치면 **skipped 인데 회복 카드가 달린** 행이 커밋됐다.
  이제 그 실행의 회복 카드 행을 잠그고 판정한다 — 먼저 온 결정 하나만 적히고, 뒤 요청은
  기존 계약대로 **409 `RECOVERY_ALREADY_DECIDED`**.
- **재배치 승인**: 두 요청이 모두 "블록 없음"을 보고 같은 회복 카드에 블록을 두 개 만들었다.
  이제 회복 카드 행을 잠그고 판정한다 — 두 요청 모두 **200 + 같은 `scheduledBlockId`**.
- 잠금 단위는 실행 1건(결정) · 회복 카드 1건(승인)이다. 결정은 생성 잠금(v2.31)을 잡지
  않으므로 생성 중인 요청과 서로 막지 않는다.
- FE 영향: 없음. 겹친 결정의 두 번째 응답이 409 인 것은 순차 재호출과 같은 동작이다.

---

## v2.31 — 2026-09-23 (같은 실행에 동시 generate 가 와도 세트는 한 벌, #481)

**동작 보정(스키마·응답 형태는 그대로) — `POST /recovery/proposals/generate` 의 멱등이
동시 호출에서도 성립한다.** 마이그레이션 없음. 새 에러 코드 없음.

- "이미 pending 카드가 있으면 그대로 반환" 이 **순차 호출에서만** 참이었다. 동시 요청은
  pending 조회를 나란히 통과해 각자 LLM 을 부르고 각자 세트를 INSERT 했다 — 실측: 동시
  2회 → 세트 2벌·LLM 2회, 3회 → 3벌·3회. 그 뒤 재조회는 카드 4장·6장(같은 그룹 중복)을
  돌려줬다.
- 이제 실행 행을 트랜잭션 동안 잠가 그 실행의 **생성만** 직렬화한다. 뒤따르는 요청은
  기다렸다가 같은 pending 세트를 그대로 받는다 — 동시 요청에도 **201 + 같은 `attemptId`**,
  새 409 는 없다. 같은 사용자의 **다른** 실행은 서로 막지 않는다.
- FE 영향: 같은 실행에 요청이 겹쳐도 카드 수가 2~4장 계약을 벗어나지 않는다. 필드·값 집합
  변경은 없다.
...
요청·응답 스키마 무변경, 새 에러 코드 없음, 마이그레이션 없음.

### 목표가 자리표시자뿐이면 422 (planB-1)

첫 질문에서 [충분해요]를 누르면 outcome 에는 `(미입력 목표)` 자리표시자만 남는다. 예전엔
그걸로 LLM 이 일반론 20세션을 지어냈고(미러 실측 "이번 구간 핵심 방향성 정의하기"…), 승인하면
소속시킬 목표가 없어 0건이 저장되는데도 200 이 나가 온보딩이 목표·카드 없이 끝났다.

- 이제 두 엔드포인트 모두 **LLM 을 부르기 전에** 422 `COMMON_VALIDATION_ERROR`,
  `field="goals.list"`, 메시지 "계획을 세울 목표가 아직 없어요. 하고 싶은 일을 한 가지만
  알려 주시면 거기서부터 세울게요."
- 이미 저장된 그런 Draft 의 승인(0건 저장)은 종전 그대로다(#88).

### `aiSource` 는 분해 기준, 폴백 계획은 스스로 밝힌다 (planB-4·5)

- `aiSource="rule"` 은 이제 **분해**가 룰 폴백일 때만이다. 분해는 LLM 이 했는데 검토만
  타임아웃나도 '룰 기반' 으로 표시되던 것을 바로잡았다.
- 분해가 폴백하면 `warnings[0]` 이 사유별로 **사용자가 할 수 있는 일만** 말한다.
  지연·일시 오류면 "이번엔 AI가 세부 내용을 만들지 못해 칸만 잡아 뒀어요 — 카드를 눌러 직접
  채우거나 잠시 뒤 다시 만들어 보세요.", 예산 소진이면 "…내일 다시 만들어 보세요.",
  **금지어·톤 게이트·프롬프트 누락이면 다시 만들기를 권하지 않는다**("이번엔 AI가 이 목표의
  세부 내용을 만들지 못해 칸만 잡아 뒀어요 — 카드를 눌러 하고 싶은 내용으로 직접 채워
  주세요.") — 같은 목표·같은 프롬프트면 다시 불러도 같은 자리에서 걸려, 하루 생성 횟수만
  쓰기 때문이다. 그 계획엔 "'이어가기' 회차 N개를 덧붙였어요"·"4주까지만 잡아요"·확정
  마일스톤 누락 안내가 더는 붙지 않는다(자리표시자 계획에서는 사실과 달랐다).
- 분해가 타임아웃·예산·금지어/톤 게이트처럼 **다시 불러도 같은** 사유로 폴백했거나 그래프가
  이미 90초를 넘겼으면 검토·재분해를 새로 시작하지 않는다. LLM 이 멈춘 날의 대기가 최악
  4.5~8분에서 분해 한 번(≈2.3분)으로 준다. **단 이 ≈2.3분은 위 '다시 불러도 같은' 사유일
  때다** — 검증 실패·제공자 오류처럼 다시 걸어 볼 만한 사유로 90초 직전에 폴백하면 검토
  한 바퀴(최악 3×45초 ≈135초)가 그대로 시작돼 총 ≈3.7분이 될 수 있다.

### 계획 안내 문구 (planB-3·9·10·11·13·14, journey-9)

스키마 무변경. `warnings[]` 문장과 초안 트리가 사실에 맞게 바뀐다.

- 목록을 싣는 안내는 제목 뒤에 조사를 붙이지 않는다("'운동 시작'는" → "이번 계획에 넣지 않은
  목표: '운동 시작' — …"). 'Focus 3 · Maintain 5'·'parked'·"을(를)" 같은 내부 표기와 ISO 날짜
  ("2026-10-15 까지고")는 "집중 목표 3개·유지 목표 5개 … 보류", "10월 15일까지" 로 나간다
  (해가 다르면 "2027년 8월 30일").
- "…까지 채우려고 '이어가기' 회차 N개를 덧붙였어요" 는 마감이 계획 지평(4주·만다라 2주) 안일
  때만 마감 날짜를 말한다. 아니면 "이번 계획 구간(N주)까지".
- 마감이 계획 첫날이면 항목별 "배치할 가용 시간을 찾지 못했어요" 줄 대신 원인과 새 마감을
  정하는 길을 말하는 한 줄이 나간다(몰아 잡았을 때도 알린다).
- 하루 과부하 안내는 배치와 같은 상한으로 판정한다(집중 시간 240분 사용자의 120분 카드 계획이면
  "평소 기준(3시간)").
- 분해·마일스톤이 사용자 목표 제목을 옮겨 쓴 자리는 금지어 치환 전 원문으로 남는다
  ('포기하지 않고 …' 가 '잠깐 쉬어가는하지 않고 …' 로 저장되지 않는다). LLM 이 쓴 문장은
  종전대로 치환된다.
- LLM 이 트리에 없는 `nodeId` 로 낸 카드는 root 아래 leaf 노드로 붙어 나온다 —
  `actionItems[].nodeId` 는 언제나 `goalNodes[].nodeId` 중 하나다.

### '주 N회' 목표의 배치 간격 (planB-6)

`scheduledBlocks[]` 의 **날짜 분포**가 바뀐다. 개수·길이·응답 모양은 그대로다.

- 빈도를 말한 목표(`goals.frequency`)는 배치 창을 **세션 개수 기준**으로도 잡는다 —
  `올림(세션 수 × 7 ÷ 주 N회)` 일이 분 기준 창보다 넓으면 그쪽을 쓴다(계획 지평 상한은 그대로).
  예전엔 분 기준 창이 좁아 '주 5회' 20세션이 한 달력 주에 6회씩 들어가고 '매일' 이 하루 두 번이
  되곤 했다.
- ⚠️ **달력 주(월~일) 단위 개수 상한은 걸지 않는다.** 마감 계획의 세션 수는 '주 N회 ×
  올림(일수/7)' 이라, 주 중간에 시작·끝나는 창(마감 계획은 대개 그렇다)에서 주 단위로 막으면
  넘친 몫이 짧은 첫·끝 주의 **하루에 여러 개로 쌓인다**(실측: 토요일 시작 16일 창 '매일' →
  일요일 하루 5개). 하루에 몰아 잡느니 어느 한 주가 N+1 회가 되는 편이 낫다고 보고, 간격은
  위의 창 넓히기로만 지킨다.

### 분량 부족 안내가 인용하는 숫자 (planB-7)

- 길이·빈도만 답해 주당 시간이 **반올림으로 채워진** 목표는 그 반올림 값 대신 사용자가 실제로
  고른 두 답을 인용한다 — "주 2시간 쓸 수 있다고 하셨는데" → "매일 20분씩(주 2.3시간) 하고
  싶다고 하셨는데"(시간은 소수 한 자리까지. 예전엔 '주 2.33333시간' 이 나갔다). 주당 시간을
  직접 답한 목표는 종전대로 그 값을 인용한다.
- 한 번에 집중 가능한 시간을 **아무 데서도 답하지 않아 기본값(50분)** 인 경우는 사용자 말처럼
  인용하지 않고 "아직 몰라서 기본값 50분으로 잡았거든요" 라고 밝힌다.

### 긴 제목은 잘라서 저장한다 (interview-10)

인터뷰 답에는 길이 제한이 없어, 한 문단으로 답하면 그 문장이 통째로 제목이 됐다(실측 214자).
예전엔 그대로 INSERT 해 **인터뷰 마지막 턴이 500**, 계획 승인이 재시도 끝에 500
`PLAN_SAVE_FAILED` 로 끝났다.

- 이제 저장 직전에 **낱말 경계에서 자르고 `…` 를 붙인다** — 목표 제목 200자, 단계(`goalNodes`)·
  마일스톤 제목 200자, 카드(`actionItems`) 제목 300자. 한도 이하 제목은 **한 글자도 손대지
  않는다**(기존 행과의 대조 키가 그대로다).
- 잘린 값은 `GET /goals`·`GET /goals/{goalId}/nodes` 등 제목을 싣는 모든 응답에 그대로
  보인다. 제목은 목표 화면에서 사용자가 언제든 고칠 수 있다.
- 같은 인터뷰 답은 항상 같은 결과로 잘린다 — 재사용 대조(같은 목표 다시 쓰기, 다음 주기 시드,
  마일스톤 재승인 멱등)가 이 값을 키로 쓴다.

### FE 가 할 일

- 이 422(`field="goals.list"`)를 받으면 목표 입력(인터뷰)으로 되돌린다. '목표 분류 확인'
  화면에서 자리표시자만 있으면 다음 단계로 보내지 않는 것도 같은 뜻이다.
- `aiSource="rule"` 을 '오프라인 모드' 로 부르지 말고 `warnings[0]` 문장을 그대로 보여 주면 된다.
- 계획 생성이 30초를 넘기면 "평소보다 오래 걸리고 있어요" 같은 안내를 띄운다(서버 최악 대기는
  여전히 2분 이상일 수 있다).
- 마일스톤 확인 화면도 `aiSource` 를 읽는다 — `"rule"`(AI 가 못 만들어 준비·진행·마무리 3단계로
  채운 것)이면 그 사실을 알리고 '다시 만들기' 를 내어 준다. `MilestoneListResponse.aiSource`
  타입에 `"saved"`(지난 주기에 확정한 뼈대를 그대로 돌려준 정상 경로, v1.82)가 빠져 있다.
- `POST /plans/generate` 는 멱등이 아니다 — `api.ts` 의 `Idempotency-Key` 주석이 그 반대로
  적혀 있다.

---

## v2.30-planA — 2026-09-22 (계획 초안 편집이 승인에 반영된다 외)
...
- **블록 편집의 정책 검사가 승인·재계획과 같아졌다**(동작 수정, 계약 무변경). `PATCH
  /plans/{planId}/blocks/{blockId}` 는 DB `time_policies` 만 봤는데, 그 행을 만드는 FE 화면이 없어
  실사용자는 늘 빈 목록이었다 — 편집기는 사실상 아무것도 막지 않았다(실측: 활동 시간대를 08:00~16:00
  이라 답한 사용자가 블록을 03:00 으로 옮겨도 200). 같은 사용자가 `POST /plans/{planId}/approve` 에서는
  02:00 이 422 였다. 이제 편집도 재계획과 **같은 조립**을 본다: DB 정책 + 최근 완료 인터뷰(설정에서
  고친 값 우선)의 활동 시간대 밖 + no_touch, 인터뷰가 없으면 설정의 활동 시간대, 그마저 없으면
  23:00~08:00 기본 수면창. 에러 코드·문구는 그대로다(422 `POLICY_VIOLATION`, "이 시간대는 수면 시간과
  겹쳐요. 다른 시간으로 옮겨 주세요.", `field="startAt"`). 활동 시간대 안의 이동은 종전 그대로 200.
- 같은 판정에서 시각 `"24:00"`(하루 끝)을 스케줄러와 같게 읽는다 — 예전엔 그 값을 못 읽어 그 정책을
  통째로 건너뛰었고, 활동창 여집합의 저녁 조각(예: 16:00~24:00)이 한 번도 검사되지 않았다.
- **FE 가 할 일**: 주간 그리드에서 활동 시간대 밖을 회색 칸으로 그리면(승인 화면과 같은 규칙) 사용자가
  끌어 보고 나서야 422 를 만나지 않는다.
- `GET /plans/weekly` 의 날짜 형식 오류 문구에서 요청 필드 이름을 뺐다(동작·코드 무변경):
...
## v2.30-interview — 2026-09-22 (인터뷰가 끝나야 할 때 끝나고, 답한 값이 그대로 남는다)
...
### 묻지 않은 활동 시간대가 하루 전체를 가두지 않는다

- 자유서술 답에 섞인 시간대 이야기에서 `time.activity_window` 를 더 이상 **추론하지 않는다**.
  실측: "새벽에만 집중이 돼서 밤에 작업해요" 한 마디로 00:00~06:00 이 활동창으로 채워졌고,
  한 번도 묻지 않은 그 값이 그 사용자의 **유일한 배치 가능 시간**이 됐다(내 정보에도 그대로
  저장). 계획은 22:00 에 일정을 잡아 놓고 "활동 가능 시간과 겹치지 않아서" 라고 스스로를
  반박했다. 질문이 "이 시간 밖엔 일정을 안 잡아요" 라고 약속하는 경계라, 확인받지 않은 값이
  정의하면 안 된다.
- `goals.weekly_time` 도 같다 — 길이×빈도로 유도되면 **묻지 않기로 한** 슬롯인데 수확만 그
  규칙을 보지 않아 말없이 채워졌다.
- 두 슬롯은 이제 열린 채 남아 정식 질문으로 묻는다 → 그 세션의 `ambiguityScore` 가 예전보다
  늦게 줄 수 있다(묻는 문항이 그만큼 남는다). 응답 형태·에러 코드는 무변경.

### 주당 시간이 확인 카드와 계획에서 같은 값이다

- `outcome.coreGoals[].weeklyHours` 와 확인 카드(`summary.timeSummary`)가 같은 계산을 쓴다.
  예전엔 카드가 "약 주 3.5시간(한 번 30분 × 매일)" 이라 확인받고 목표 기록은 주 2시간이라,
  사용자가 [이대로 진행] 을 누른 숫자와 계획이 쓰는 숫자가 달랐다.
- 주당 시간을 **직접 답한** 세션에서는 카드가 곱셈 결과 대신 그 답을 그대로 말한다. 길이·빈도만
  답해 주당 시간을 묻지 않은 세션에서는 종전대로 "약 주 N시간 (한 번 … × …)" 이 보인다.
  응답 형태는 무변경.

### 칩 질문에 직접 적은 시간이 가까운 보기로 반올림되지 않는다

- 칩 슬롯은 FE 가 직접 입력도 받는다("직접 입력해도 돼요…"). 거기 적은 **길이**가 가장 가까운
  보기로 반올림돼 저장됐다 — `goals.session_length` 에 "20분" 을 적으면 30분, "45분" 도 30분,
  "7분" 은 15분, "하루종일" 은 240분. 세션 길이는 모든 블록의 길이이자 주당 분량의 기준이라,
  계획 전체가 사용자가 말한 적 없는 숫자 위에 세워졌다.
- 이제 **길이 보기를 가진 슬롯**(`goals.session_length`·`goals.weekly_time`·
  `recovery.downscope_unit`·`energy.focus_duration`)에서는 보기와 **같은 길이**일 때만 그 보기로
  받는다. 표기가 달라도 길이가 같으면 그대로 받는다 — "1시간30분"·"90분" → `1시간 30분`,
  "한 시간" → `1시간`, "한 번에 2시간 정도요" → `2시간`.
- 보기로 맞출 수 없는 답은 **'없음' 으로 닫지 않고 보기를 들고 한 번 더 묻는다**(고정 보기가
  있는 칩·select 전부에 해당). 되묻는 문구는 답을 탓하지 않고, 임의로 가까운 보기를 대신
  고르지 않는다. 같은 답을 상한(3회)까지 되풀이하면 기존 best-effort 경로로 진행해 같은 질문이
  무한히 반복되지 않는다.
- FE 영향: 같은 `slotKey` 의 질문이 한 번 더 올 수 있다(`currentQuestion.options` 그대로).
  응답 형태·에러 코드는 무변경.
...
## v2.30-goals — 2026-09-22 (목표·습관·인박스 정합성)
...
## v2.30-today — 2026-09-22 (오늘 실행 루프 — 이어서 하기·자정 넘긴 카드·취소 블록 정리·쪼갠 카드·정지 멱등·실제 소요 시간)
...
## v2.30-recovery — 2026-09-22 (회복 카드 이름·날짜·재관여 알림 바로잡기)
...
## v2.30-sched — 2026-09-22 (주간 리뷰·cron 신뢰성)
...
## v2.30-calendar — 2026-09-22 (캘린더 연결이 조용히 끊기지 않게 · 고정 일정 겹침 409·24:00)
...
- 자정을 넘는 일정(`22:00–02:00`)은 여전히 422 지만, 이유와 입력 방법을 말한다: "자정을 넘기는 일정은
  ...
  **시작·종료를 거꾸로 적은 범위(11:00–10:00)에는 그 안내가 나가지 않는다** — 자정을 넘겨 읽으면
  23시간짜리 일정이라 수업·알바일 수 없다. 이제 "종료 시각이 시작 시각보다 늦어야 해요. 시작과
  종료가 바뀌지 않았는지 확인해 주세요." 로 갈린다(같은 422 `COMMON_VALIDATION_ERROR`, 같은
  `field="startTime"`). 갈림값은 자정을 넘겨 읽었을 때의 길이 12시간 — 그 이내면 종전 안내 그대로다.
...
## v2.30-auth — 2026-09-22 (인증·개인정보·푸시 감사 수정)
...
## v2.30-llm — 2026-09-22 (AI 톤 필터가 사용자 문구를 고치지 않음 · LLM 가드 정비)
...
## v2.30-materials — 2026-09-22 (영상 커리큘럼에서 비공개·삭제 영상 제외)
