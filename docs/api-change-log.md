...
- 분해가 폴백하면 `warnings[0]` 이 "이번엔 AI가 세부 내용을 만들지 못해 칸만 잡아 뒀어요 —
  카드를 눌러 직접 채우거나 잠시 뒤 다시 만들어 보세요."(예산 소진이면 "…내일 다시 만들어
  보세요.")다. 그 계획엔 "'이어가기' 회차 N개를 덧붙였어요"·"4주까지만 잡아요"·확정 마일스톤
  누락 안내가 더는 붙지 않는다(자리표시자 계획에서는 사실과 달랐다).
- 분해가 타임아웃·예산·금지어/톤 게이트처럼 **다시 불러도 같은** 사유로 폴백했거나 그래프가
  이미 90초를 넘겼으면 검토·재분해를 새로 시작하지 않는다. LLM 이 멈춘 날의 대기가 최악
  4.5~8분에서 분해 한 번(≈2.3분)으로 준다.
