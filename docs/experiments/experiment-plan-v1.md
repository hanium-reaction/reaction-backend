...
| P7 | `notification_sends.target_action_item_id` + `opened_at` | ⚠️ **BE 는 됐고 값이 안 들어온다** — 컬럼 둘 다 실재하고(`db/models/notification_send.py:76,84`) 콜백 엔드포인트 `POST /notifications/{id}/opened` 도 있다(`routes/notifications.py:227`). 다만 **`opened_at` 은 FE 가 그 콜백을 부르기 전까지 영원히 NULL** 이고, 그 FE 작업은 `reaction-frontend#268` 로 **아직 열린 PR** 이다. ⚠️ §11 항목 19 는 이것을 그냥 "✅ 완료" 로 적어 두 절이 어긋나 있었다 — 이 행이 맞다 | 근접 효과 지표, 알림 실험 전부 | M |
...
| P10 | IRB 면제/간이 심의 **회신** | ⚠️ **발송 완료(2026-08-21), 회신 대기** — §7.1·§11 항목 1 과 어긋나 있던 것을 이 행에 맞춘다. 회신이 와야 W4 L3 게이트가 닫힌다 | **L3 전부** | — |
...
### L1-8. 실행 이력이 계획을 바꾸는가 (오프라인)

L1-6 은 **자료**를, L1-7 은 **인터뷰에서 말한 제약**을 덮는다. 사용자가 실제로 **한 것**
(실패 사유·회복 결과·연속 실패)이 다음 계획을 바꾸는가는 잴 자가 없었다. 그 신호 셋이
#466·#467 로 배선됐고, **셋 다 효과가 측정된 적이 없다.**
...
| ⚠️ **강한 지표와 약한 지표를 섞지 말 것** | M37·M38 은 케이스 단위 이진 판정이라 42건으로도 결론이 된다. M36 은 LLM 분해의 확률성에 묻히므로 **단발 실행으로 읽지 않는다.** 반복해도 분모는 **케이스 수(6쌍)** 이지 실행 수가 아니다 — M29 가 겪은 함정과 같다 |
| ⚠️ **`failure_hard_to_start` 는 탐색용** | "first_step 이 쉬워졌는가" 를 결정적으로 셀 방법이 없다. 사람 라벨이 붙기 전까지 성공 기준에 넣지 않는다 |
| **실패 시 해석** | M37 > 0 이면 프롬프트의 "인용 금지" 규칙이 안 먹는 것이다 — 톤 사고라 **효과 논의보다 먼저** 고친다. M36 이 0 근처면 "이력은 프롬프트에 실렸지만 계획을 안 바꾼다" 는 뜻이고, 그건 #466 의 투자 근거가 약하다는 결과이므로 이슈에 그대로 적는다 |
| ⚠️ **감쇠의 도달 범위가 좁다 (실측)** | 골든셋을 구우며 확인했다 — 분량 감쇠(#467)는 (a) 사용자가 **빈도를 명시했으면 분량에 무효**하고(`target_sessions_per_week` 우선순위 1 이 density 가감을 막는다), (b) 빈도 미언급이어도 표준 밀도 목표가 한 호출 세션 상한에 걸리면 총 분량·총 세션 수가 클램프돼 `sessions_per_week` 와 하루 집중 상한만 움직인다. 이 골든셋의 여섯 목표는 **감쇠가 무는 구간**에 있도록 골랐고 테스트가 그 사실을 프로덕션 함수로 고정한다. 나머지 두 구간을 재려면 별도 블록이 필요한데, 그건 "감쇠를 어디까지 물게 할 것인가" 라는 **제품 결정이 먼저**다 |
| **알려진 한계** | ① 전량 합성이라 실사용 이력 분포를 대표하지 않는다 — 실제 도그푸딩 데이터는 `skipped`("나중에")가 대부분인데 그 값은 계획 집계에서 **일부러 제외**된다(#469). ② M40 은 위 도달 범위 때문에 **감쇠가 무는 구간에서만** 뜻이 있다 |
| **하네스** | `scripts/l1_8_run.py` — 프롬프트 변수·이력 포맷·감쇠를 전부 **프로덕션 함수로** 만든다(L1-7 1차가 `review_feedback` 을 빼먹어 34호출이 프로덕션 아닌 프롬프트였던 전례). 채점은 순수 함수라 `tests/test_l1_8_scoring.py` 가 **실행 없이** 검증한다 |
| **실행 결과** | ⚠️ **1차 결론 철회 (2026-09-07)** — 1차 126호출 + 2차 162호출, 폴백 0. [`l1-8-results.md`](l1-8-results.md) §4. **재현된 것**: 톤 지표(M37·M38 = 0, 약 200건)와 **룰 감쇠**(−174 / −225). **철회된 것**: "이력 신호가 분량을 반대로 민다"(1차 +62~93 → 2차 −7~+1). 원인은 **간헐적 반토막 생성** — 54케이스 중 13건(24%)에서 반복 하나가 그 케이스 중앙값의 70% 미만이었다. 6쌍 × 3회로는 ±50~100분 효과를 못 가린다 |
| **소요 / 담당** | 골든셋 0.5일(완료) + 하네스 0.5일(완료) / AI-1 |
...
| **M36** | **`history_volume_delta`** | — | (이력 처치군 **총** 계획 분량) − (**같은 목표** 대조군의 값), 분 | — | — | **L1-8 1차 지표.** ⚠️ 블록 평균끼리 빼지 않는다 — 짝(`pair_id`) 단위로 빼고 그 분포를 본다. ⚠️ **v2(2026-09-07)에서 '평균 세션 분' → '총 분량' 으로 바꿨다** — 세션 길이는 사용자 입력이 고정해 처치에 구조적으로 반응할 수 없었다. 변경 근거와 스모크 실측은 [`l1-8-results.md`](l1-8-results.md) §1 |
...
| 주 | **구현 트랙** | **실험 트랙** | 게이트 |
|---|---|---|---|
| **W1** | ✅ **3/3** — P4·P5·P6 컬럼(`09fa61fbf06f`) · P9(ADR 로 이미 해소돼 있었음) · 신규 전략 4종 시드 + 핀 반전 | ✅ **5/5** — 골든셋 120건·P1·루브릭·사전등록 완료. IRB 는 **2026-08-21 발송**했고 회신 대기다(레포 밖, W4 게이트). ⚠️ 이 칸은 오래 "IRB 만 미착수" 로 남아 §7.1·§11 항목 1·§1 P10 과 어긋나 있었다 | ✅ **충족** — 골든셋 해시 + 루브릭(`rubric-v1.md`) + 사전등록(`preregistration-v1.md`) 확보 |
...
| 골든셋 **84건** | ✅ | `eval/golden_first_plan_cases.jsonl` + 생성기 + 무결성 테스트 20개. ⚠️ **초판은 66건이었다** — M29 대조군을 12 → 30 으로 넓히면서 84건이 됐고, 이 표가 그동안 낡은 수를 들고 있었다. 단일 진실 소스는 파일과 [`eval/README.md`](../../eval/README.md) 다 |
...
| **M26-core 산출** | ✅ **0.794 (27/34)** | 배치 경로(#418)를 분해 하네스에 배선해 **같은 계획 위에서** M17~M25 를 합쳤다. `--summarize-only` 로도 나온다(LLM 재호출 없음). 지표별: M17 0.853 · M19 0.941 · M20 0.941 · M21 1.000 · M22 1.000 · M23 1.000(**적용 6건**) · M24 1.000(**적용 4건**) · M25 1.000. ⚠️ **TravelPlanner 4.4% 와 직접 비교 금지** — 우리 제약은 대부분 독립 체크라 더 쉽다. ⚠️ 배치는 **'달력이 빈 사용자' 조건**이라 M20·M21 이 낙관적이다. ⚠️ M22 는 **안전 불변식**이라 성과 주장에 안 쓴다. ⚠️ **이 0.794 를 M33 문서의 arm 별 M26-core(general `A_none` 0.941)와 나란히 읽지 말 것** — 두 수는 파이프라인의 **다른 지점**을 잰다. 여기 저장분은 ③층 **보정 전 원안**이고 M33 저장분은 ③층을 통과한 계획이라, M33 데이터에서는 M17 이 구조적으로 실패할 수 없다(밴드 밖 leaf 0/34 대 5/34). [#464](https://github.com/hanium-reaction/reaction-backend/issues/464) — 바로 아래 「계측 구멍」이 그 원인이다 |
...
| **M33 실행 (3-arm)** | ✅ **2026-09-04** | [`m33-3arm-results.md`](m33-3arm-results.md) — general 102행 · challenge 48행, 룰 폴백 0건. **M33 = +0.0000** (양쪽, CI [0,0]). ⚠️ **이것은 「④층이 효과 없다」가 아니다** — general 은 반려 0/34 라 처치 노출 자체가 없었고(Δ 가 정의상 0), challenge 는 처치가 있었지만 반려 사유(의미 품질)와 M26-core 실패 축(m20·m21)이 만나지 않았다. **estimand mismatch** 다. [#433](https://github.com/hanium-reaction/reaction-backend/issues/433) 판단 근거로 **쓰면 안 된다.** ⚠️ **이 실행이 아래 「다음」의 순서를 뒤집었다** — 원래 이 행은 "리드타임이 달라 M33 을 먼저 낸다"는 권고였는데, 결과 문서 §9 는 **"결과 변수만 바꿔 다시 실행하면 안 된다 — 무엇을 결함으로 볼지가 먼저"** 로 방향을 바꿨다. M34·M30·M31 은 여전히 미측정(v4 의 구조화 finding 을 먹는 재분해 배선이 없다) |
...
7. 🟡 **L1-4 — 하네스만 완료, 본 실행 미착수.**
    ⚠️ 초판은 이 항목을 "✅ 예비 실행 완료" 로 적었는데, §7 타임라인은 같은 것을
    "미착수" 로 적어 두 절이 반대로 읽혔다. **정확히는 도구가 준비됐을 뿐 1,800 호출
    본 실행은 안 돌았다** — 아래 본문이 스스로 한정하던 내용을 제목으로 끌어올린다. `scripts/report_llm_run_metrics.py` 신설 — `llm_runs` 를 module×prompt_version 으로 묶어 p50/p95 지연·fallback rate·`reason` 원인 분해(계획서 3분해와의 대응표 포함)·토큰·비용을 집계한다. `.github/workflows/report-llm-run-metrics.yml` (workflow_dispatch, 선례와 동일 EC2 러너)로 아무 때나 돌릴 수 있다. **다만 라이브에 v2 트래픽이 얼마나 쌓였는지는 미확인** — 실행은 준비됐고, 실제 dispatch 로 숫자를 뽑는 건 별개 할 일이다.
...
19. ✅ **`notification_sends` 확장 — `target_action_item_id` + `opened_at` — BE 완료.**
    ⚠️ **BE 한정이다.** 컬럼과 콜백 엔드포인트는 있지만 `opened_at` 은 FE 가 그 콜백을
    부르기 전까지 항상 NULL 이고, 그 FE 작업(`reaction-frontend#268`)은 아직 열린 PR 이다.
    그래서 §1 의 P7 은 ✅ 가 아니라 ⚠️ 다 — **P7 행이 단일 진실 소스**다.
