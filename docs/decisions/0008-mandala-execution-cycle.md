...
> **정정 메모(2026-09-18, v2.30)**: 폴로 옮긴 뒤에도 `force=False` skip 때문에 **18:00 첫 폴의
> 스냅샷이 그 주 내내 잠겼다** — 21:00 회고 알림 뒤의 체크인이 리포트에 안 들어갔다. 이제 일요일
> 폴은 매번 다시 집계(`force=True`, 결정적 upsert)하고, 늦은 회고(월·화)까지 담는 확정 집계는
> 매일 04:30 `weekly_review_finalize` 가 회고 창이 닫힌 주(다음 주 목요일 00:00 이후)에 한 번
> 한다. `GET /reviews/weekly` 는 확정본만 저장값으로 쓰고 그 전엔 즉석 계산한다.
...
| **F** | 저녁 회고 알림 일요일 분기(문구·딥링크). 새 클래스 없음 ✅(2026-08-24) | 0 | E |
| **G** | 다음 2주 Draft 제안 + 승인 (ADR-0007 PR-5, 만다라 2주 스코프로 축소) ✅(2026-08-24) | 0 | D·E |
| **H** | 손 못 댄 축 축소 제안 ✅(2026-08-25, 마일스톤 재조정은 미포함 — 아래 참고) | 0 | G |
...
> **완료 메모(2026-08-24, "F")**: 발송 조건("회고할 카드가 있을 때만", 클래스 dedup, 주
> ≤3건 예산)은 요일과 무관하게 그대로 뒀다 — §4 가 "클래스·예산·중복 규칙은 그대로"라고
> 못 박았고, "일요일 저녁 알림은 어차피 나간다"는 문장은 구조적 보장이 아니라 경험적
> 가정이라 그대로 믿었다(가드를 느슨하게 하면 새 발송 조건을 만드는 셈이라 §8 "새 클래스
> 없음"의 정신에 어긋난다). `title`/`body`/`url` 세 필드만 요일로 갈라진다 — 딥링크는
> `/reviews/weekly`(기존 "/reflection" 프리픽스가 `/reflection` 라우터를 그대로 미러하는
> 것과 같은 관례). push body 엔 리포트 숫자를 넣지 않았다 — 실제 내용은 딥링크 도착지가
> 보여주고, 여기서 만다라 조회를 추가하면 이미 사용자 순회 중인 sweep 루프에 쿼리가
> 하나 더 붙는다. 구현: `scheduler/notify_sweeps.py::_evening_payload`.

> **완료 메모(2026-08-24, "G")**: 표의 "ADR-0007 PR-5" 참조는 문면 그대로는 PR-3(진척
> 롤업)·PR-4(전환 제안 판정, 주간 리뷰 응답)가 먼저 있어야 한다 — 둘 다 그 문서에서
> 아직 미구현이다. 그 일반형(마일스톤 있는 임의 목표, N주 주기)을 여기서 같이 짓지
> 않았다 — 이 스코프(PR D 의 만다라 2주 rolling 창)는 마일스톤 층이 아예 없고 주기 길이가
> 항상 고정 2주라 훨씬 좁다. 대신 ADR-0007 §5 의 "잔여 세션 판정"(날짜가 아니라 완료
> 여부)을 마일스톤 가드만 뺀 채 재사용해 `orchestrator/cycle_proposal.py` 로 새로 짰다.
>
> 승인은 정말로 **기존 것을 그대로** 쓴다 — `POST /plans/generate` 를 빈 바디로 부르면
> `_resolve_outcome` 우선순위 ③(최근 완료 인터뷰 재투영)이 같은 heaviest 목표를 다시
> 잡고, `_max_plan_weeks` 가 다시 2주로 캡한다(PR D). 새 생성 엔드포인트는 0개.
>
> **가장 까다로웠던 지점**: `ActionItem` 은 원본 status 불변(AGENTS §2)이라 재생성돼도
> 종결(done/failed 등) 카드는 archive 되지 않고 영구히 남는다. goal 전체를 대상으로
> "종결 카드가 있다"를 물으면 첫 주기 이후 영원히 참이 돼버려 "아직 아무것도 안 함" 가드가
> 무의미해진다. `should_propose_next_cycle` 입력을 **현재 활성 계획 트리(`tree_kind='plan'`)
> 의 leaf 에 매달린 action_item만**으로 좁혀 해결했다 — `_archive_goal_nodes` 가 매 주기
> leaf 트리를 archive 하므로 이 필터가 자연히 과거 주기를 걸러낸다. real DB 테스트
> (`test_cycle_proposal_real_db.py`)로 이 격리를 직접 확인했다 — fake session 은 `goal_node_id
> IN (...)` 을 실제로 실행하지 않아 이 버그를 원리적으로 잡을 수 없다.

> **정정(2026-08-25, "G" 가드①)**: 위 판정에 **제안이 영영 안 뜨는 결함**이 있었다.
> 가드①("남은 카드가 있으면 아직")이 `target_date` 를 안 봐서, 날짜가 지난 미종결 카드도
> "남은 카드"로 셌다. 문제는 `expire_unreflected` cron 이 `completion_status='in_progress'` 인
> **실행(execution_events)** 만 대상으로 한다는 점이다 — 한 번도 [▶시작] 하지 않은 `planned`
> 카드는 execution_event 자체가 없어 **영원히 쓸려나가지 않는다.** 결과적으로 체크를
> 흘려보내는 사용자(= 이 제품이 도우려는 바로 그 사용자)는 2주가 지나도 밀린 카드 한 장 때문에
> 다음 주기 제안을 영영 못 받았다.
>
> 가드①을 **`target_date >= today` 인 미종결 카드만** 세도록 고쳤다 — 날짜가 지난 미종결
> 카드는 '남은 일'이 아니라 '밀린 일'이고, 재계획이 **다뤄야 할 대상**이지 재계획을 막을
> 근거가 아니다(`orchestrator/replan.py` 도 과거 미착수 블록을 배정으로 치지 않는다).
> 가드②("종결 카드가 하나도 없으면 제안 안 함")는 그대로 뒀다 — 전부 밀렸고 끝낸 게 하나도
> 없는 사용자에게 필요한 건 새 2주가 아니라 지금 계획의 조정이다(§5 의 원래 취지).
> `should_propose_next_cycle(action_items, *, today)` 로 시그니처가 바뀌었다(시계는 호출자가
> 주입 — 이 모듈의 "DB·시계 무관" 규약).

> **완료 메모(2026-08-25, "H")**: 표의 "ADR-0007 PR-6 마일스톤 재조정 포함"은 이번에
> 같이 짓지 않았다 — "G" 완료 메모에서 이미 밝혔듯 PR-6 이 기대는 마일스톤 재조정
> HITL(§10, "MilestoneDraft 에 id 필드 신설 필요")은 그 자체로 first_plan/milestone 생성
> 흐름을 건드리는 별도 작업이고, ADR-0007 의 PR-3·4·5(일반형 주기 전환) 트랙이 아직
> 없어 "재조정할 주기 자체"가 없다. 대신 §6 이 "이미 있다"고 명시한 수정 수단만으로
> 제안 카드를 만들었다 — `PATCH /goals/mandala/nodes/{id}`(축/칸 텍스트),
> `POST /plans/mandala/{planId}/regenerate-branch`(축 재생성, 승인 전 초안 한정). 이
> 카드는 새 엔드포인트가 0개다.
>
> "3주 연속"을 어떻게 셌나: `period_summaries` 에 아무것도 안 쌓는다는 §7 의 원칙을
> 그대로 지켰다 — 매 조회 시 `compute_weekly_stat` 을 최근 3개 주(이번 주·지난 주·2주
> 전) 각각에 대해 계산해 `untouched_axis_ids` 교집합을 구한다
> (`mandala_adapter.compute_stale_axes`). `untouched_axis_titles`(문자열)만으로는
> `PATCH` 로 제목이 바뀌면 주 사이 매칭이 끊기므로 이번에 `untouched_axis_ids` 를
> `MandalaWeeklyStat` 에 새로 추가했다. 막 만든 축이 "지난 주 데이터 없음"만으로 방치
> 취급되지 않게 축의 `created_at` 이 판정 창의 가장 오래된 주보다 앞서야 한다는 가드를
> 넣었다 — 없으면 축 재생성 직후 새 축이 곧바로 "3주째 방치" 취급된다.
>
> `nextCycleProposals`(G)와 달리 이 판정은 `ActionItem`/plan 트리를 전혀 안 본다 —
> 승격·계획 여부와 무관하게 만다라 트리 자체(습관 체크인 + `completed_at`)만으로
> 판정하므로, HTTP 라우트 테스트에서도(습관 없는 프로젝트형 칸 한정) 실제 로직을
> `_FakeSession` 한계 없이 검증할 수 있었다(G/E 의 raw-session 한계와 다른 지점).
