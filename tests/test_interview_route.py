...
def test_start_returns_first_question(client: TestClient, monkeypatch: Any) -> None:
    ...
    assert body["ambiguityScore"] == 16  # 미해결 필수 슬롯 수 (goals.approach 추가 #approach)
...
def test_submit_advances_and_persists(
    ...
    assert body["ambiguityScore"] == 15  # 하나 채워져 감소 (필수 16개, #approach)
