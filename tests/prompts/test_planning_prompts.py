...
생략한 `prompt_id` 를 `latest()` 로 해석하므로 버전 없이 부르는 호출은 새 버전 파일만으로
갈아탄다. 버전이 여럿인 프로덕션 호출부는 이제 `@vN` 으로 핀돼 있지만(`tests/test_prompt_version_pins.py`),
후보 버전도 핀을 올리는 순간 프로덕션이 되므로 존재하는 **모든 버전**을 함께 검사한다.
